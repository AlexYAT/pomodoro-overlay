...
Собранный **один файл** `PomodoroOverlay.exe` — в [Releases](https://github.com/YOUR_USERNAME/pomodoro-overlay/releases) на GitHub (замените ссылку после публикации).
...
```bash
git clone https://github.com/YOUR_USERNAME/pomodoro-overlay.git
cd pomodoro-overlay
python -m venv .venv
...
git init
git add .
git commit -m "Initial release: Pomodoro Overlay Timer"
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/pomodoro-overlay.git
git push -u origin main
```

Релиз с portable exe:

1. Соберите `dist\PomodoroOverlay.exe`
2. GitHub → **Releases** → **Draft a new release** → прикрепите `PomodoroOverlay.exe`
